...
void tmPlanner::updateMap(const cv_bridge::CvImagePtr& cv_image_ptr,
  ...
  grid_map_.updateMapFromImage(
      ...
      sensor_parameters_.coefficient_B);
  std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
...
geometry_msgs::Pose tmPlanner::findNextBestGridPoint(
  ...
  double cov_trace_prev;
  if (planning_parameters_.use_threshold) {
    cov_trace_prev = simulated_grid_map.computeCovarianceTrace(
  ...
  for (const auto& evaluated_point : lattice_.getLatticePoints()) {
    ...
    evaluated_grid_map.predictMapUpdate(
        ...
        sensor_parameters_.fov_angle_x, sensor_parameters_.fov_angle_y,
        sensor_parameters_.coefficient_A, sensor_parameters_.coefficient_B);

    // Calculate gain (reduction in covariance trace).
    double cov_trace_final;
    if (planning_parameters_.use_threshold) {
      cov_trace_final = evaluated_grid_map.computeCovarianceTrace(
          planning_parameters_.lower_threshold);
    } else {
      cov_trace_final = evaluated_grid_map.computeCovarianceTrace();
    }
    auto gain = cov_trace_prev - cov_trace_final;
...
double tmPlanner::optimizeControlPoses(const double* x, const int N) {
  ...
  double cov_trace_final;
  if (planning_parameters_.use_threshold) {
    cov_trace_final = simulated_grid_map.computeCovarianceTrace(
        planning_parameters_.lower_threshold);
  } else {
    cov_trace_final = simulated_grid_map.computeCovarianceTrace();
  }

  gain = cov_trace_prev - cov_trace_final;
